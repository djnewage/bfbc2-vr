...
| So `"Render.ForceFov"` is reachable | the `Render` module *is* the live `GameRenderSettings` instance; `ForceFov` is field `+0x18`, `Float32`. No string for it exists because nothing in the shipped game asks for it — the resolver does not need one | above + `reflection-bfbc2.txt` |
...
- Whether writing `Render.ForceFov` through `GetGlobalVariable` replaces the FOV hunt: untested. The call is `0x004F4C40`, two stack args (name, `Float32` TypeInfo object `0x01BF13E4`); the calling convention needs confirming from the call-site disassembly before the mod calls it. Read-only first (`gvar` verb), then one write.
- Why the shape scans found no `GameRenderSettings` instance when the resolver proves one exists — probably its booleans are mostly 0 in this build and the first, loosest scan's 73 hits contained it (`3C4437E4`: `NearPlane=0.1000`). Re-check that address's vtable next launch.
