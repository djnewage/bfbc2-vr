...
{
    const int v = camover::on_draw(_ReturnAddress(), false, c);
    if (v == 1) return D3D_OK;
    if (v == 2) vrcomp::hud_begin_draw(); else vrcomp::hud_end_draw();
...
{
    const int v = camover::on_draw(_ReturnAddress(), true, pc);
    if (v == 1) return D3D_OK;
    if (v == 2) vrcomp::hud_begin_draw(); else vrcomp::hud_end_draw();
